...
df_batch = pd.DataFrame(columns= ['test_id', 'test_type',
                                  ...
                                  'water_table_depth',
                                  'well_name', 'well_radius', 'casing_radius', 'screen_length',
                                  'screen_top_depth', 'test_data_file'])
...
df_batch['test_data_file'] = [os.path.join(test_data_folder, f"test_{i+1}.csv") for i in range(number_of_tests)]
...
df_results = run_batch(batch_data = batch_file,
