...
def readData():
    ...
    filename = input("Enter file name of data file: ")
    try:
...
def calculateKhBouwerRice(rc, rw, Le, T0, T0plus, T0first, d, Bt, aniso):
    ...
    A = (
        ...
        - 0.00008148 * (b_r) ** 2
        + 0.0000001028 * (b_r) ** 3
        - 0.00000000006484 * (b_r) ** 4
        + 0.00000000000001573 * (b_r) ** 5
    )
    B = (
        0.2372
        + 0.005151 * (b_r)
        - 0.000002682 * (b_r) ** 2
        - 0.0000000003491 * (b_r) ** 3
        + 0.0000000000004738 * (b_r) ** 4
    )
    ...
    coeff2 = (
        (1.1) / (math.log((d + b) / r_asterisk)) + ((A + (B * coeff)) / b_r)
    ) ** -1
    # print 'coeff2: ', coeff2
...
# from the paper
D = 80  # aquifer thickness
H = 5.5  # Distance from water table to the bottom of the screen
L = 4.56  # screen length
rc = 0.076
rw = 0.12
####
...
d = 80  # z-position of top of screen (positive direction downward)
...
def generate_slug_test_data(h0, t0, num_points=20, noise_level=0.01, seed=123):
    """
    Generate synthetic Bouwer & Rice slug test data.

    Parameters:
    h0 : float
        Initial head displacement [m]
    t0 : float
        Characteristic time constant [T]
    num_points : int
        Number of time points
    noise_level : float
        Standard deviation of noise to add (fractional)
    seed : int or None
        Random seed for reproducibility

    Returns:
    DataFrame with 'time', 'head', 'ln_h_over_h0'
    """
    if seed is not None:
        np.random.seed(seed)

    time = np.linspace(0, 5 * t0, num_points)
    h_t = h0 * np.exp(-time / t0)
    min_val = np.min(h_t)
    # Add some random noise
    noise = np.random.normal(0, noise_level * h0, size=h_t.shape)
    h_t_noisy = h_t + noise

    # Avoid negative heads due to noise
    h_t_noisy = np.clip(h_t_noisy, 0, None)
    mask = h_t_noisy == 0
    h_t_noisy[mask] = min_val

    ln_h_over_h0 = np.log(h_t_noisy / h0)

    df = pd.DataFrame({"time": time, "head": h_t_noisy, "ln_h_over_h0": ln_h_over_h0})
...
df = generate_slug_test_data(h0, t0, num_points=30, noise_level=0.02, seed=42)

# Plotting
plt.figure(figsize=(10, 5))
plt.subplot(1, 2, 1)
plt.plot(df["time"], df["head"], "o-")
plt.xlabel("Time [min]")
plt.ylabel("Head [m]")
plt.yscale("log")
plt.title("Slug Test Head Recovery")
####
# Read data, convert pressure to waterpressure, convert date to seconds, reads filename
# data = readData()
seconds = df["time"].values.tolist()
head = (1 - df["head"]).values.tolist()
...
if 0:
    ...
    stableLvl = int(stableLvl)
    baseLvl = np.mean(
        head[head.index(minHead) - stableLvl - 10 : head.index(minHead) - stableLvl]
    )
baseLvl = 100.0
...
print(
    "Horizontal hydraulic conductivity based on all data: Kh = ",
    "%.8f" % Kh_cms_full,
    "cm/s or ",
    "%.5f" % Kh_md_full,
    "m/d",
...
print(
    "Horizontal hydraulic conductivity based on data in interval [0.15-0.25]: Kh = ",
    "%.8f" % Kh_cms_part,
    "cm/s or ",
    "%.5f" % Kh_md_part,
    "m/d",
...
print(
    "Horizontal hydraulic conductivity based on data in interval [0.3-1]: Kh = ",
    "%.8f" % Kh_cms_first,
    "cm/s or ",
    "%.5f" % Kh_md_first,
    "m/d",
...
print("**************")
print(
    "Horizontal hydraulic conductivity based on all data and Kz/Kh = 1: Kh = ",
    "%.8f" % Kh_BR1_cms,
    "cm/s or ",
    "%.5f" % Kh_BR1_md,
    "m/d",
...
print(
    "Horizontal hydraulic conductivity based on all data and Kz/Kh = 0.1: Kh = ",
    "%.8f" % Kh_BR2_cms,
    "cm/s or ",
    "%.5f" % Kh_BR2_md,
    "m/d",
...
print(" ")
print(
    "Horizontal hydraulic conductivity based on data in interval [0.20-0.30] and Kz/Kh = 1: Kh = ",
    "%.8f" % Kh_BR1_cms_plus,
    "cm/s or ",
    "%.5f" % Kh_BR1_md_plus,
    "m/d",
...
print(
    "Horizontal hydraulic conductivity based on data in interval [0.20-0.30] and Kz/Kh = 0.1: Kh = ",
    "%.8f" % Kh_BR2_cms_plus,
    "cm/s or ",
    "%.5f" % Kh_BR2_md_plus,
    "m/d",
...
print(" ")
print(
    "Horizontal hydraulic conductivity based on data in interval [0.3-1] and Kz/Kh = 1: Kh = ",
    "%.8f" % Kh_BR1_cms_first,
    "cm/s or ",
    "%.5f" % Kh_BR1_md_first,
    "m/d",
...
print(
    "Horizontal hydraulic conductivity based on data in interval [0.3-1] and Kz/Kh = 0.1: Kh = ",
    "%.8f" % Kh_BR2_cms_first,
    "cm/s or ",
    "%.5f" % Kh_BR2_md_first,
    "m/d",
...
def writeLogfile():
    logText = "\n".join(
        [
            "##############################################################",
            "Logfile Slug Test Analysis Script",
            "##############################################################",
            " ",
            " ".join(["Time:", time.strftime("%a, %d %b %Y %H:%M:%S ")]),
    ...
    logText = (
        ...
        + " cm"
        + "\n"
        + "Initial Displacement H_0 = "
        + "%.2f" % H_0
        + " cm"
        + "\nDuration Slugtest: "
        + "%.1f" % slugTestSeconds[endTest - 1]
        + " s\n\n"
    )
    logText = (
        logText
        + "Figure 0 is a H vs t plot of the entire head data set.\n"
        + "***Saved as "
        + filename[:-4]
        + "_fig0.pdf***\n\n"
    )
    logText = (
        logText
        + "Figure 1 is a normalized head vs t plot of the slugtest head data.\n"
        + "***Saved as "
        + filename[:-4]
        + "_fig1.pdf***\n\n"
    )
    logText = (
        logText
        + "Figure 2 is a normalized head vs log t plot of the slugtest head data.\n"
        + "***Saved as "
        + filename[:-4]
        + "_fig2.pdf***\n\n"
    )
    logText = (
        logText
        + "Figure 3 is a log normalized head vs t plot.\n"
        + "***Saved as "
        + filename[:-4]
        + "_fig3.pdf***\n\n"
    )
    logText = (
        logText
        + "Four regression lines are plotted:\n\n"
    ...
    logText = (
        logText
        + "2) Regression based on normalized head data in interval 0.15-0.25.\n"
        + "Regression Equation: y ="
        + " %.13f" % a_Hvorslev
        + "x +"
        + " %.11f" % b_Hvorslev
        + "\nR² = "
        + "%.4f" % float(r2_Hvorslev)
        + "\n\n"
    )
    logText = (
        logText
        + "3) Regression based on normalized head data in interval 0.20-0.30.\n"
        + "Regression Equation: y ="
        + " %.13f" % a_BR
        + "x +"
        + " %.11f" % b_BR
    ...
    logText = (
        logText
        + "4) Regression based on normalized head data in interval 0.3-1.\n"
        + "Regression Equation: y ="
        + " %.13f" % a_first
        + "x +"
        + " %.11f" % b_first
    ...
    logText = (
        logText
        + "1) Calculation for anisotropy ratio (Kz/Kh) of 1:\n"
        + "-------------------------------------------------\n\n"
    )
    logText = (
        logText
    ...
    logText = (
        logText
        + "b) Calculation based on normalized head data in interval 0.15-0.25.\n\n"
        + "H_0+ = "
        + "%.2f" % H_0plus
        + " cm\n"
        + "T_0+ = "
    ...
    logText = (
        logText
        + "c) Calculation based on normalized head data in interval 0.3-1.\n\n"
        + "H_0+ = "
        + "%.2f" % H_0plus3
        + " cm\n"
        + "T_0+ = "
    ...
    logText = (
        logText
        + "2) Calculation for anisotropy ratio (Kz/Kh) of 0.1:\n"
        + "---------------------------------------------------\n\n"
    )
    logText = (
        logText
    ...
    logText = (
        logText
        + "b) Calculation based on normalized head data in interval 0.20-0.30.\n\n"
        + "H_0 = "
        + "%.2f" % H_0plus2
        + " cm\n"
        + "T_0 = "
